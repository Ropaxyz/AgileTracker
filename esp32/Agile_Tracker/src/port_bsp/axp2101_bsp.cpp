...
void Custom_PmicRegisterInit(void) {
  ...
  // MX1.25 LiPo: enable PMIC battery detect + ADC (required for voltage / %)
...
static int voltageToPercent(uint16_t mv) {
  // Single-cell LiPo: ~3.3 V empty, ~4.2 V full
  if (mv >= 4200) {
    return 100;
  }
  if (mv <= 3300) {
  ...
  return static_cast<int>((static_cast<int32_t>(mv) - 3300) * 100 / (4200 - 3300));
}
...
int Axp2101_GetBatteryPercent() {
  const uint16_t mv = axp2101.getBattVoltage();
  const bool connected = axp2101.isBatteryConnect() || (mv >= 3100 && mv <= 4500);
  if (!connected) {
    return -1;
  }

  const int pct = axp2101.getBatteryPercent();
  if (pct > 0 && pct <= 100) {
    return pct;
  }
  if (mv >= 3100) {
